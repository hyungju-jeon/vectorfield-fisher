...
def plot_current_state(
    ...
    if "fisher" in plot_list:
        ...
        axs[idx].set_xlim(-2.5, 2.5)
        axs[idx].set_ylim(-2.5, 2.5)
        axs[idx].set_title("Fisher Information Map")
        idx += 1
...
if __name__ == "__main__":
    ...
    if os.path.exists(initial_model_file) and not run_initialize:
        ...
        ensemble_dynamics = env.EnsembleRBF(rbf_grid_pts, n_models=10, device=device)
    ...
    model_list = ["uncertainty_perturb"]
    for model_id in model_list:
        exp_params, icem_params, _ = get_param()
