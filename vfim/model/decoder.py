...
class Decoder(nn.Module):
    """Base class for decoders in a variational inference framework.
...
class LogLinearNormalDecoder(Decoder):
    ...
    def __init__(self, dx, dy, device="cpu", l2=0.0, C=None, b=None, sigma_obs=None):
        """
        Initializes the NormalDecoder with input and output dimensions and the device to run on.
        Args:
          dx (int): The dimensionality of the input features.
        ...
        if sigma_obs is not None:
            self.logvar = nn.Parameter(
                torch.log(torch.tensor(sigma_obs, device=device).expand(1, dy)),
                requires_grad=False,
            )
        else:
            self.logvar = nn.Parameter(
                torch.ones(1, dy, device=device), requires_grad=True
            )
